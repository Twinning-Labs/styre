...
- **Named residuals:** owned-extension tooling files lose cross-stack fan-out (rung-3); manifests in `paths` vestigial for routing; staleness softened from hard-error to advisory sweep + note. **Positive-trace nuance (overall-review):** the sweep records only on *failure*, so a passing/empty sweep (e.g. a single-stack repo where `untouched` is empty) leaves no positive evidence that an unowned file was present-but-not-specifically-gated — "nothing rides through silently" holds for *breakage* surfacing but not as a positive audit trail; a WO-6 surfacing item.
