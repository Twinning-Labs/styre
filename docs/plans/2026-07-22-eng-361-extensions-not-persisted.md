...
**Accepting 3 is safe for any profile written after ENG-362** — the derived extension list equals the stored one for every kind whose `kind` was not agent-refined (Task 1 asserts the two tables agree), and both are empty for an unrecognised kind. So no deployed profile needs regenerating. A profile written *before* ENG-362 whose `kind` was refined by the discovery agent may route differently; that residual is listed in the behavior changes and is repaired by re-running `styre setup`.
