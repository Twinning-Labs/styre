...
**Status:** proposed (brainstorm, converged after operator review + git archaeology + live-plan grounding)
...
1. **Scope of Fix B (needs a decision).** Surfacing `files_to_touch` (+ `test_plan`) to implement is a prompt-var addition, not a wording edit — larger than the approved "tell implement its declared tests are its own." Fix A ships correctly without it (worst case: one under-delivery loopback per unit that declares a test implement wouldn't have written unprompted). Options: **(i)** both, as specified; **(ii)** Fix A + the §4.2/§4.3 wording only, accepting the loopback; **(iii)** Fix A now, Fix B as its own ticket. Recommendation: **(i)** — implement being gated on files it is never shown is a defect independent of this ticket, and it is the mechanism by which Fix A would otherwise cost a cycle.
