...
- **Deterministic prior (from the M2b-persisted exit code + framework + rawOutput).** `checks:classify` reads the `{exit_code, framework}` M2b now persists (§2b) — no re-derivation — and settles a coarse observation: *passed* / *errored-before-running* (pytest exit 2 collection/import error, a Go build error, jest "Cannot find module", …) / *assertion-failed* (pytest exit 1 with an assertion in the trace). It resolves only the **unambiguous** cases on its own: a clean `ImportError`/`NameError` of the AC's **own new** symbol ⇒ `absence`; M2's coarse `error` (couldn't-attempt) ⇒ `environmental`. The exit code makes the prior robust (not a brittle text-only parse); it remains only a *prior* — the semantic classification below is the adjudicator's.
  - **Read contract (append-only signals):** `ground_truth_signal` is append-only, so a scoped re-author leaves the *previous* round's `ac-check-red-first` signal(s) behind with a now-dangling `detail_json.acCheckId`. M3 must read the RED-first signal **by the live `ac_check.id`** (join `signal.detail_json.acCheckId → ac_check.id` for the current row), never "the latest signal for the AC" — else it classifies on a stale trace. This needs a net-new `signalForAcCheck(acCheckId)`-style query.
...
- **One shared M3 adjudicator dispatch** handles all judgment: the semantic `red_class` for ambiguous reds (§3) **and** the green-on-HEAD per-check disposition (§4). **The prior never self-resolves `assertion`** (§3) — an assertion-failed check is by definition ambiguous (it may be a proxy-absence) and **always reaches the adjudicator**. The agent-skip optimization therefore fires only when **every** check was prior-settled via the two `absence`/`environmental` rules (i.e. all clean own-symbol import errors / couldn't-attempts — rare); any assertion-red or green check forces a dispatch.
...
- **Classify touches only un-classified rows; already-classified rows are immutable.** On a first pass, `checks:classify` classifies every check in one transaction (a crash before commit rolls back — no mixed-vintage partial write). On a **re-author round**, it (re)classifies **only the freshly re-authored rows** (the flagged ACs' new `ac_check` rows, which have NULL `red_class`); every previously-classified row is **frozen** (§6 write-once). This is load-bearing: if classify re-ran the *nondeterministic* adjudicator over already-resolved siblings, a non-flagged `already-satisfied` check could flip to `vacuous`, changing the flagged set so the `(ac_id, "vacuous")` signature never repeats → escalate never trips → budget burn (the v1 failure scoping exists to prevent). Freezing classified rows keeps the flagged set monotone-shrinking and the escalate bound (§7-terminal) sound. "Recompute all in one txn" applies **only** to a single classify invocation's crash-resume, never across re-author rounds.
