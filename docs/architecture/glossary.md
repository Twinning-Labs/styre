...
**ground truth** — a verdict produced by an objective mechanism: build output, test results, CI
...
**idempotency key** — a globally unique token, constructed by prefixing the `dispatch_id` (e.g.
...
**K (concurrency cap)** — the limit on concurrent in-flight tickets the daemon will advance at once.
The default at cutover is K=2, set via `orchestrator.max_concurrent_features`. (minimal-loop.md §4)

**loopback** — a reset that sends a ticket back to an earlier stage when a gate fails, so the
...
**needs-you inbox** — the queue of tickets parked on a `human_resume` signal, requiring an operator
...
**next_step_key** — the routing key computed by the deterministic resolver to select the next step
...
**open-core seam (projector contract)** — the set of versioned, stable interfaces the commercial
...
**projection_outbox** — the table that holds pending outward writes (to Linear and GitHub). Every
...
**projector** — the component that drains `projection_outbox` and applies each row idempotently to
...
**signal** — an inbound ground-truth fact delivered to the loop: e.g. checks green
...
**stage** — the ticket's position in the lifecycle. `ticket.stage` is one of exactly six values:
...
**work_unit** — a per-`kind` decomposition of the implement stage (kinds include `backend`,
...
**workflow_step** — a durable journal entry representing one logical unit of progress, identified by
