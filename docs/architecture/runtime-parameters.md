...
| `75` | paused (`EX_TEMPFAIL`) | **Any** paused run (`exitCodeForOutcome`) — reason `budget`, `needs_you` (including a PR the forge did not deliver, and an agent dispatch whose confinement could not be confirmed — ENG-476, the pause names the cause — a resume retries the request; one made for an older commit is replaced when the PR step runs again at the new head), or `interrupted`. The checkpoint (SoT + transcript) is already on disk and **no retry attempt is consumed**. Also returned by `styre clean <ident>` when that ident is currently a live run — `clean` refuses rather than reaping. | Paused: yes — `styre run --resume <ident> --profile <p>`. `clean` on a live run: not as-is — wait for the run to finish or pause, then clean. |
