...
|----|------|------|--------|----------|
| A1 | Ground-truth verification | BLOCK | **DECIDED** | Objective + EXTERNAL to the agent (no self-report). Layered: build → tests (existing + the plan's `test_plan` realized as real tests; **behavioral ⇒ test required, non-behavioral exempt; design stage classifies**) → diff⊆scope (advisory → reviewer) → independent reviewer → CI green (required check = merge arbiter; PR #184 hermeticity is a prereq). Runs in the worktree via project-profile commands. **[2026-07-18 annotation — scoped to the commercial plane]** CI-as-merge-arbiter is the **commercial Control Plane**'s outer loop (it owns the human-merge wait and can treat a required check as blocking that gate). In the **OSS** `styre run`, CI is *reported*, not gated: build/tests/diff⊆scope/independent-reviewer remain hard ground-truth gates as decided above, but CI is a best-effort t+0 read emitted as `ci_handoff` telemetry — OSS exits PR-ready regardless of the read. See `docs/brainstorms/2026-07-18-checks-report-not-gate-design.md` and the §11 changelog entry below. |
...
| D3 | Escalation surface / needs-you inbox | LATER | **REOPENED (split, 2026-07-11)** | **Reopened for the outbound-notify half only** (§11 changelog 2026-07-11): a vendor-neutral `NotifierPort` (Slack = first adapter) + policy dial (`notify: escalations\|transitions\|everything`) ships **in the OSS core**, wired into `styre run` (fail-loud config validation at startup + outbox-driven delivery on escalate/terminal). The **SQLite-backed queue / `styre inbox` / two-way Slack (buttons→decision)** stays commercial-plane, per the OSS/commercial boundary (2-way requires a long-lived listener process, which a one-shot `styre run` cannot be). Linear stays the human tracking projection either way. |
...
## 11. Changelog
- **2026-07-11 — Slack outbound notifications shipped in the OSS core; D3 reopened (split).** Design: `docs/brainstorms/2026-07-11-slack-notifications-design.md` (status: implemented). The frozen design placed all of "Slack notify" on the commercial side (D3, `minimal-loop.md` needs-you inbox). This reopens **only the outbound half**: a vendor-neutral `NotifierPort` (mirrors `AgentRunner`/`selectIssueTracker`) with Slack as the first adapter, a `notify` policy dial (`escalations`/`transitions`/`everything`, default `escalations`), delivery via the existing `projection_outbox` (enqueue-in-tx, sweep + terminal + post-loop drain, seq-keyed idempotency, never escalates a notify-row transport failure), and **eager fail-loud config validation** (`assertSlackConfigured`, wired into `src/cli/run.ts` right after runtime-config discovery, before any ticket work — covers both the fresh-run and `--resume` paths) plus a one-line stderr startup confirmation when a notifier is active. Rationale for the split: outbound is a pure one-way sink (violates no invariant, a solo `styre run` user benefits directly); the **two-way listener** (Slack buttons → decision) genuinely requires a long-lived process across human-response latency, which stays commercial-plane (the needs-you inbox, `styre inbox`, SQLite-backed queue). Known v1 narrowing (explicit, not silent): message omits `ticketTitle`/`links`; the `blocked` dead-end terminal is exit-code-only (avoids double-notifying an escalation-blocked ticket); Slack render is mrkdwn text, not Block Kit — each a trivial later enrichment. D3 row updated above to record the split.
- **2026-06-26 — OSS/commercial boundary made consistent across all docs; supersedes the pre-boundary "daemon = OSS" framing.** Operator confirmed the two-repo open-core model: **`Twinning-Labs/styre`** = the OSS execution core (GPLv3); a **future, not-yet-created private `Twinning-Labs/control-plane`** = the commercial SaaS. The plane runs the **persistent daemon** (multi-ticket pickup, K-concurrency scheduling, the needs-you inbox, dashboards) by invoking `styre run` as a subprocess + reading its NDJSON stdout telemetry + integrating only through the versioned seam (build-operations §5); it never forks/imports the core. **This supersedes** the 2026-06-20 entry's "three run modes (setup / daemon / run)" and "daemon = OSS solo/local model": the shipped OSS binary is `run` / `setup` / `migrate` (no `styre daemon`), and the **persistent daemon is the commercial plane**. The earlier framing was pre-boundary; the *code* (src/index.ts, src/cli/, src/cli/park.ts exit-75 resume) is ground truth and the docs are now aligned to it. A documentation consistency pass (plan: `docs/plans/2026-06-26-oss-boundary-doc-consistency.md`) reframed the OSS-facing docs (README + `docs/architecture/{README,glossary,execution-model}.md`) and the frozen deep spec (`control-loop.md` / `minimal-loop.md` / `projector.md` — banners + daemon→"the runner" for the inner per-ticket loop; the outer multi-ticket loop, K, needs-you inbox, launchd/host-service supervision fenced as **commercial Control Plane**, design record preserved not deleted), plus `build-operations.md`, `CLAUDE.md`, `CONTRIBUTING.md`. Also added the missing README **Install** (Homebrew: `brew install twinning-labs/styre`) + corrected **Develop** (git clone + macOS Xcode-CLT/codesign prereq). **Note on §12 below:** its "16 tables" count is stale — the shipped schema has **14 active tables + 1 commented `-- DEFERRED` `memory_record` stub** (verified in both `schema.sql` copies); §12 is preserved as historical record (this log is append-only), the corrected count lives in `docs/architecture/README.md` + `glossary.md`.
- **2026-06-23 — metrics layer finalized: `metric_event` retired as a carry (optional, plane-derived); cache-token cost capture added to `dispatch`.** Operator questioned whether the long-listed "`metric_event` writer" carry was real, given cost already exports. Confirmed it is NOT: all dollar cost originates in agent dispatches, and the `dispatch` table already records per-dispatch `cost_usd`/`tokens_in`/`tokens_out`/`cache_*`, which M9 already emits to the stdout stream + sums into the run summary. `metric_event` is a redundant *denormalized* rollup of `dispatch` + `event_log` that a dashboard could prefer — the **plane derives it from the emitted rows; the OSS core does not write it.** Relabeled the table in schema.sql (both copies) as an explicit DEFERRED/OPTIONAL stub, rewrote the misleading "needs a metric_event writer — deferred" comment in `run-ticket.ts`, and corrected build-operations §5.3 (the implemented stream emits `dispatch` rows, not `metric_event`). Separately closed the one genuine gap the audit had flagged: `parseClaudeJson` now parses `cache_read_input_tokens`/`cache_creation_input_tokens`, `AgentRunResult`+`completeDispatch` thread them, and the telemetry `dispatch` event + `summary` carry `cache_read`/`cache_create` (the columns existed but nothing populated them). **Net: the OSS metrics/telemetry layer is complete; no remaining OSS carries** (the complexity grader stays built-but-default-off). Code: `src/agent/{runner,providers/claude,fake-runner}.ts`, `src/db/repos/dispatch.ts`, `src/dispatch/run-dispatch.ts`, `src/telemetry/{events,emitter}.ts`.
- **2026-06-23 — per-ticket "ticket contract" DEFERRED TO THE COMMERCIAL PRODUCT; OSS input contract is title+description+type.** Operator decided the entire rich-ticket-ingestion layer named in build-operations §4/§5 — the **`styre_config` block** (`max_loop_cycles`/`strictness`/`test_command`/`target_branch`), the **AC checklist**, the **context-files list**, and the **"Ready for Agent" trigger** — is **not OSS work**; the plane owns it end-to-end. None of it is built (the `IngestedTicket` contract reads only ident/title/description/typeLabel/uuid/url). Rationale: (1) `styre run` is **one-shot**, so the caller (a solo script OR the plane) already sets per-run config via the normal `--config`/RuntimeConfig path at launch — a ticket-embedded settings block the core must parse is redundant; it was designed for a hand-authoring solo dev and for a daemon's auto-pickup, neither of which is OSS scope. (2) AC criteria + context-file hints are already expressible as **prose in the ingested `description`**, which feeds the design agent — nothing is lost; the structured forms are a plane-side enrichment. (3) "Ready for Agent" is a continuous-pickup gate = scheduling = the plane's value (build-operations §2; the M7b daemon+inbox drop, §11 2026-06-22 / M7 scope). Pointer notes added at build-operations §4 and §5.1. **Consequence:** the only genuinely-remaining OSS carry is the `metric_event` writer (forensic/non-blocking); the complexity grader is built but default-off. This refines (does not contradict) the §4 "four-tier config" / §5 seam — the per-ticket tier and the Linear-ticket-contract seam item are now plane-owned, not OSS-built.
...
- **2026-07-18 — OSS `merge:await-checks` deleted; checks are reported, not gated (report-not-gate).** The resolver's checks-gate branch, the checks poller, `[CL-POLL]`'s polling loop, the `external_checks` signal type, and the wait-budget columns (`attempts`/`max_attempts`/`first_attempt_at` as they applied to checks) are removed from OSS `styre run`. In their place: a single best-effort t+0 read of the project's checks-system state (`readCiState`, bounded by `CI_READ_TIMEOUT_MS`≈8s, never throws, never blocks) fires on the merge path right after the PR opens, is emitted as a non-journaled `ci_handoff` telemetry event, and `styre run` exits PR-ready immediately after — **including on a `failing` read** (D1: the read is informational, not a gate). CI-watch-and-reconcile beyond that single read is the **commercial Control Plane**'s outer loop (fenced, undesigned, same status as S9 human-merge). Rationale: the OSS run is a one-shot CI/fleet primitive; idle-burning a process to poll a checks system it cannot act on (no auto-merge; the human decides) bought nothing but wall-clock and complexity, and the removed wait-budget machinery was the same shape of thing Task 5's collateral cleanup also removed from `run-ticket.ts`. Docs reconciled across `control-loop.md` (S8 rewrite; atlas P1/P2/P3 rows removed §8.3; §7.3 signal table/prose; S9 boundary note + guard; §11 worked example; residual mentions), `minimal-loop.md` (`next_step_key` pseudocode, flow diagram, `POLL_INTERVAL` retired), `execution-model.md`, `projector.md`, `glossary.md`, `README.md`, and this file's A1 row (annotated above, not rewritten). Design: `docs/brainstorms/2026-07-18-checks-report-not-gate-design.md`; plan: `docs/plans/2026-07-18-checks-report-not-gate-plan.md`.

- **2026-09-20 — bounded review repair and explicit risk acceptance.** Supersedes the historical immediate-escalation treatment of nominated major findings. Major/critical findings remain blocking; repair authors must answer finding IDs with evidence and an independent reviewer must explicitly resolve them. Review-origin loopbacks and runner-mediated verification requests each have a durable ticket-wide limit of three. Plan deferrals are invalid. Resume retries unresolved review; accepting eligible code-review risks requires explicit IDs, rationale, and unchanged reviewed SHA. Accepted risks are audited and disclosed in the PR; the production forge pushes the reviewed object. Legacy checkpoints preserve proven completed-round semantics and retain ambiguous unresolved debt. Maintained contracts: `docs/architecture/review-repair.md`.
...
| `metric_event` *(deferred/optional — no writer; plane derives from emitted `dispatch`/`event_log`)* | §6 | metrics/events.jsonl |
