...
- **In-feature, remaining:** Ruby/PHP/`prepare` (M-C3); AGENTS.md command source DONE (WO-4, commit `de4c80d`); CI-reading (scoped to command extraction) + precedence wiring still pending the Q2 pilot; file-identity rung-1 + the Bash-allowlist/test-file re-expression; gates/triggers + run-more-when-unsure + never-claim-verified-on-gap; Python/Go/JVM non-root via the identity model; **stack-grounded design/extract decomposition (WO-13) — item 1 (feed detected stacks into the planner) DONE; the `kind`/coupling half deferred to WO-5/M-D (§9 item 8).**
