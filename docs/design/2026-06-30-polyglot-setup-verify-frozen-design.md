...
The early code answered only #1/#3, and answered them with a brittle abstraction (below).
...
- **In-feature, remaining:** Ruby/PHP/`prepare` (M-C3); CI-reading (scoped to command extraction, pending the Q2 pilot) + AGENTS.md; file-identity rung-1 + the Bash-allowlist/test-file re-expression; gates/triggers + run-more-when-unsure + never-claim-verified-on-gap; Python/Go/JVM non-root via the identity model.
...
- **Out of this feature (reframe / other milestones):** per-verify recompute persisted-and-kept-live (commercial); the transitive cross-stack dependency graph + coupled-cluster dispatch + design-time human gate (**control-loop milestone — touches the closed S1–S10 catalog**); the pre-PR interactive hold (commercial); OSS env-bubble (the `run`-loop terminal behavior).
