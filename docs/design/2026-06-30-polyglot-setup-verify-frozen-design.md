...
- **In-feature, remaining:** Ruby/PHP/`prepare` (M-C3); AGENTS.md command source DONE (WO-4, commit `de4c80d`); CI-reading (scoped to command extraction) + precedence wiring still pending the Q2 pilot; **file-identity routing + run-all safety DONE (WO-5, commits `0b941c0`/`6384811`)**; the WO-6 cost refinement (inert-skip + `sweep-cost` measurement DONE — commits `d29f138`/`7759451`; the named hard-gate global-file set + the T1 over-budget bound remaining); Python/Go/JVM non-root via the identity model; **stack-grounded design/extract decomposition (WO-13) — item 1 (feed detected stacks into the planner) DONE; the `kind`/coupling half deferred to WO-5/M-D (§9 item 8).**
...
1. **T1 cost-feasibility (fulcrum).** Measure run-all on a real polyglot fixture at the frequency the coarse classifier triggers it; make the per-verify recompute + content-hash skip the prerequisite; implement the over-budget branch (defer expensive tier to the gap-surfaced merge) so the safe path is never silently disabled. — **INSTRUMENTED (WO-6, commit `7759451`):** every triggered sweep emits a `sweep-cost` signal (`stacksSwept`/`wallClockMs`/`unownedTriggers`, positive-trace at `stacksSwept:0`) so the cost is now *measurable*. **Still open:** the over-budget **bound** (count-based on cumulative `stacksSwept`, backstop-level default, advisory-sweep-only, surfaces `sweep-skipped-over-budget`) + content-hash skip — deferred to a calibrated fast-follow on this data.
...
3. **The seam-version change.** The routing-primitive move bumps `schemaVersion`; plan the migration of `schemaVersion: 2` profiles and the seam-contract revision. — **RESOLVED (WO-5):** shipped as v3; `parseProfile` rejects a v2 profile with a re-run message; `extensions` materialized at scan.
...
- *2026-06-30 (v4)* — **WO-5 file-identity routing + run-all safety SHIPPED** (commits `0b941c0`/`0b1e9ea`/`6384811`; TDD + per-task Opus crux + overall Opus review; 579 green). Two operator decisions: D1 materialize `extensions` at scan + bump `schemaVersion` 2→3 (closes the agent-kind-drift hole; profile self-describes routing); D2 full safety rule + docs-skip. The mixed-diff under-verify (cardinal sin) is killed via **hard gates over the owned stacks + an advisory `ran-all-unowned` sweep of untouched stacks** (surfaced, non-wedging — resolves the wedge-on-unrelated-red risk). The schema-bump (§9.1, §13 #3) is now done. Remaining for the safety pillar: the **WO-6 cost refinement** (named global-file set + T1 over-budget branch + measurement) and the passing-sweep positive-trace nuance.
- *2026-06-30 (v5)* — **WO-6 cost refinement PARTIALLY LANDED** (commits `d29f138` inert-skip, `7759451` measurement; TDD + per-task SHIP + overall outcome review; 587 green). Two safe pieces shipped: (1) the docs-skip generalized to a strict **inert-skip** (`isInertFile` — LICENSE/LICENSE.txt/NOTICE/AUTHORS/COPYING/.mailmap; `.editorconfig`/`.gitignore`/configs/lockfiles deliberately stay swept, since the predicate also feeds the zero-gate pure-inert PASS path), and (2) **sweep-cost instrumentation** — every triggered sweep emits a non-gating `sweep-cost` signal (`stacksSwept`/`wallClockMs`/`unownedTriggers`, fires even at `stacksSwept:0`), which **closes the passing-sweep positive-trace nuance** flagged in v4. The T1 cost is now **instrumented, not yet bounded**: the over-budget branch (count-based on cumulative `stacksSwept`, backstop default, advisory-sweep-only, surfaces `sweep-skipped-over-budget`), the §9.3 *tiered* branch, and **content-hash sweep dedup** are **deferred to a calibrated fast-follow** on the measured data (operator decision). Interim bound = the existing B3 wall-clock ceiling (no regression). §13 #1 updated.
