...
export function applyAcCheckGateVerdict(
  ...
  if (stillRed.length === 0) {
    // An empty still-red set means the AC checks that RAN are not red. It is NOT the claim
    // "this run verified something" — three routes reach it having measured nothing, and
    // django__django-12325 shipped a pull request down one of them.
    //
    // That check used to live here, as ENG-424's evidence floor. It has MOVED to the
    // `implement → review` transition (`evidence-floor.ts`), because here it was unreachable on
    // exactly the tickets it was written for: the resolver only schedules this step when the
    // ticket has active `ac_check` rows, and the routes that author no checks at all never
    // arrived. It also ran BEFORE `verify:integration`, so it could only have escalated runs
    // that were about to produce the very evidence it demanded.
