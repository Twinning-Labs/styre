...
- **Stopping a running agent is graceful first, so a forced kill can leave tool commands running.** The agent runs in its own process group and session. On a timeout, or when Styre receives Ctrl-C, Ctrl-\\, SIGTERM or SIGHUP, Styre sends the signal to the agent's group, waits up to 2 seconds, then kills the group. Claude Code runs each shell command in a separate group and stops those itself when asked; if it has to be killed outright, or crash recovery kills an orphaned agent's group, those commands can keep running until they finish.
- **A kill aimed at Styre's process group does not reach the agent.** Because the agent has its own session, a CI system that SIGKILLs the job's process group leaves the agent running (a SIGKILL cannot be forwarded). Prefer SIGTERM for graceful shutdown; crash recovery kills the orphan on the next resume.
