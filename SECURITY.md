...
- **A wrapper around the CLI is not reached by Styre's kills.** Styre kills the process it started. If `agent.command` points at a wrapper script or version-manager shim that runs the real `claude` as a child (rather than `exec`-ing it), a startup refusal or timeout kills the wrapper and can leave the real agent running. The default `agent.command` is the `claude` binary itself. Stopping an agent's whole process tree, including commands it is running, is ENG-485.
