...
test("postImplementAtSha: newest coarse per acCheckId at the sha", () => {
  ...
  insertSignal(db, {
    ...
    result: "fail",
    branchHeadSha: "sha1",
    detail: { acCheckId: 7, acId: 1, coarse: "red", redClass: "assertion", outcome: "gated-red" },
  });
  insertSignal(db, {
    ticketId,
    signalType: "ac-check-post-implement",
    result: "pass",
    branchHeadSha: "sha1",
    detail: { acCheckId: 7, acId: 1, coarse: "green", redClass: "assertion", outcome: "green" },
  });
...
test("advisorySweeps: only advisory:true + non-pass, newest per type, excludes gate's number[] advisory", () => {
  ...
  insertSignal(db, {
    ...
    detail: {
      advisory: true,
      ran: [
        { label: "backend:build", exitCode: 0 },
        { label: "backend:test", exitCode: 1 },
      ],
    },
  });
...
test("reauthorProvenance: newest disposition per acCheckId + joined check-wrong reason", () => {
  ...
  expect(prov).toEqual([
    { acId: 2, acCheckId: 9, disposition: "rejected", reason: "asserts 200 but AC says 201" },
  ]);
});
