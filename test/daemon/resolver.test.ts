...
test("resolver does NOT re-serve checks:arbitrate once a blame exists at the gate sha (falls through)", () => {
  const { db, ticketId } = makeTestDb();
  setTicketStage(db, ticketId, "implement");
  insertWorkUnit(db, {
    ticketId,
  ...
  const d = nextStepKey(db, ticketId);
  db.close();
  // falls through past the arbiter arm to the provision gate (not yet done).
  expect(d).toMatchObject({ stepKey: "provision" });
});
