...
test("a drive that idles to a stall pauses (needs_you) and delivers the escalation notification (post-loop drain)", async () => {
  ...
  // is never consulted → driveToTerminal idles to a stall after IDLE_CAP(3) ticks. Since ENG-383/384
  // that stall pauses via `pauseTicket` as paused(needs_you) rather than a distinct "no-progress"
  // outcome; `pauseTicket` appends an 'escalated' event, which is what actually carries the terminal
  // notification here (caught by the backstop re-sweep inside `finish()`).
